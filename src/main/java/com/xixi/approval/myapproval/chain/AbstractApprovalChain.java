...
import com.xixi.approval.myapproval.process.AbstractProcess;
import com.xixi.approval.myapproval.process.CommonApprovalProcess;
...
public abstract class AbstractApprovalChain extends AbstractProcess {
    ...
    protected AbstractNode calculateChain(List<ApprovalConfigEntity> config, List<ApprovalLogEntity> log) {
        ...
        for (Map.Entry<ApprovalConfigEntity, List<ApprovalConfigEntity>> entry : sortMap.entrySet()) {
            ...
            List<ApprovalLogEntity> collect = log.stream()
                    .filter(x -> x.getNodeIndex().equals(key.getNodeIndex()))
                    .sorted(Comparator.comparing(ApprovalLogEntity::getChildrenIdx))
